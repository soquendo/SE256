...
namespace midterm_oquendo
{
    class ValidationLibrary
    {
        ...
        public static bool movieRating(string temp)
        {
            bool result = false;

            string[] strRatings = { "G", "PG", "PG-13", "R", "NC-17" };

            foreach (string strRtngs in strRatings)
                if (temp.Contains(strRtngs))
                {
                    result = true;
                }
            return result;
        ...
        public static bool movieGenre(string temp)
        {
            bool result = false;

            string[] strGenres = { "Action", "Adventure", "Comedy", "Crime", "Drama", "Horror", "Science Fiction", "Suspense" };

            foreach (string strGnr in strGenres)
                if (temp.Contains(strGnr))
                {
                    result = true;
                }
            return result;
